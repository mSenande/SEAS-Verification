...
for var in VARNAMES:
    ...
    for icat in range(score_options[score][2]):
        ...
        if score_options[score][2]==3:
            ...
            figname = f'{DATADIR}/plots/stmonth{config["start_month"]:02d}/All_models_stmonth{config["start_month"]:02d}_hindcast{config["hcstarty"]}-{config["hcendy"]}_monthly.{aggr}.{"".join(validmonths)}.{var}.{score}{str(icat)}.png'
        else:
            fig.suptitle(f'{score_options[score][4]} \n{VARNAMES[var]}\n' + tit_line2, fontsize=16)
            figname = f'{DATADIR}/plots/stmonth{config["start_month"]:02d}/All_models_stmonth{config["start_month"]:02d}_hindcast{config["hcstarty"]}-{config["hcendy"]}_monthly.{aggr}.{"".join(validmonths)}.{var}.{score}.png'
