...
do
    ...
    for m in {1..12}
    do
        python -u 4-Verification_plots.py "$institution" "$name" $startmonth "3m" 4
        python -u 4-Verification_plots.py "$institution" "$name" $startmonth "3m" 5
        python -u 4-Verification_plots.py "$institution" "$name" $startmonth "3m" 6
    done
    wait
