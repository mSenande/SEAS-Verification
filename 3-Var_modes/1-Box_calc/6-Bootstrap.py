# %% [markdown]
# # 6. Compute score uncertainties with bootstrap

# This script is used to compute uncertainties of different verification scores
# for monthly seasonal forescasts of the North Atlantic Oscillation.
# 
...
hcnao_fname = f'{MODESDIR}/{hcst_bname}.1m.NAO.nc'
hcnao_3m_fname = f'{MODESDIR}/{hcst_bname}.3m.NAO.nc'
...
obnao_fname = f'{MODESDIR}/{obs_bname}.1m.NAO.nc'
obnao_3m_fname = f'{MODESDIR}/{obs_bname}.3m.NAO.nc'

# Check if files exist
if not os.path.exists(obnao_fname) & os.path.exists(obnao_3m_fname):
    print('No se calcularon aún los índices NAO de ERA5')
    sys.exit()
elif not os.path.exists(hcnao_fname) & os.path.exists(hcnao_3m_fname):
    print('No se calcularon aún los índices NAO de este modelo y sistema')
    sys.exit()


# %% [markdown]
# ## 6.1 Hindcast iteration over time

# Here we resample hindcast data over time, with n iterations. 

# %% 
print("6.1 Hindcast iteration over time")
...
if aggr=='1m':
    ...
    hcnao = xr.open_dataset(hcnao_3m_fname)
    obnao = xr.open_dataset(obnao_3m_fname)
else:
...
hcnao = hcnao.sel(forecastMonth=fcmonth).swap_dims({'start_date':'valid_time'})
obnao = obnao.where(obnao.valid_time==hcnao.valid_time,drop=True)
nao_hcob = xr.merge([hcnao, obnao.rename({"nao": "nao_obs"})])

# Resample hindcast data over time, with n iterations
n = 10000
nao_boot = xs.resample_iterations_idx(nao_hcob, n, 'valid_time')

h = nao_boot[['nao']]
o = nao_boot[['nao_obs']].rename({"nao_obs": "nao"})

# %% [markdown]
# ## 6.2 Probabilities for tercile categories
...
# %% 
print("6.2 Probabilities for tercile categories")
...
# %% [markdown]
# ## 6.3 Compute deterministic scores
...
# %% 
print("6.3 Compute deterministic scores")
...
# %% [markdown]
# ## 6.4 Compute probabilistic scores for tercile categories
        
# Here we calculate the probabilistic scores: area under Relative Operating Characteristic (ROC) curve, 
# Relative Operating Characteristic Skill Score (ROCSS), Ranked Probability Score (RPS), Ranked Probability Skill Score (RPSS) and Brier Score (BS). 

# %% 
print("6.4 Compute probabilistic scores for tercile categories")
...
textstr= '\n'.join((
    ...
    r'Conf. interval=({:.2f},{:.2f})'.format(float(corr_ci.nao.sel(quantile=0.025).values),float(corr_ci.nao.sel(quantile=0.975).values))))
...
textstr= '\n'.join((
    r'Reference={:.2f}'.format(score_ref),
    r'Conf. interval=({:.2f},{:.2f})'.format(float(rpss_ci.nao.sel(quantile=0.025).values),float(rpss_ci.nao.sel(quantile=0.975).values))))
props = dict(boxstyle='round', facecolor='wheat', alpha=0.5)
ax1.text(0.01, 0.99, textstr, transform=ax1.transAxes, fontsize=10,
        verticalalignment='top', bbox=props)      
# Save figure
...
textstr= '\n'.join((
    ...
    r'Conf. int.=({:.2f},{:.2f})'.format(float(roc_ci.nao.sel(quantile=0.025,category=0).values),float(roc_ci.nao.sel(quantile=0.975,category=0).values))))
props = dict(boxstyle='round', facecolor='wheat', alpha=0.5)
...
textstr= '\n'.join((
    ...
    r'Conf. int.=({:.2f},{:.2f})'.format(float(roc_ci.nao.sel(quantile=0.025,category=1).values),float(roc_ci.nao.sel(quantile=0.975,category=1).values))))
props = dict(boxstyle='round', facecolor='wheat', alpha=0.5)
...
textstr= '\n'.join((
    ...
    r'Conf. int.=({:.2f},{:.2f})'.format(float(roc_ci.nao.sel(quantile=0.025,category=2).values),float(roc_ci.nao.sel(quantile=0.975,category=2).values))))
props = dict(boxstyle='round', facecolor='wheat', alpha=0.5)
