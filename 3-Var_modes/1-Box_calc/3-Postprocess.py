...
hcnao_3m = xr.open_dataset(hcnao_3m_fname)

# Reading OBS data from file
obnao = xr.open_dataset(obnao_fname)
obnao_3m = xr.open_dataset(obnao_3m_fname)
...
for aggr,h in [("1m",hcnao), ("3m",hcnao_3m)]:
    l_probs_hcst=list()
    # For each quantile
...
for aggr in ['1m','3m']:
    ...
    for this_fcmonth in probs_hcst.forecastMonth.values:
        ...
        for icat in range(numcategories):
            ...
            # Count the number of months between the threshold
