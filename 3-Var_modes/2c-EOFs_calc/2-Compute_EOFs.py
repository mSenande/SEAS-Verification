...
hcst = xr.open_dataset(hcst_fname,engine='cfgrib', backend_kwargs=dict(time_dims=('forecastMonth', st_dim_name))).sel(latitude=slice(80,20))
...
era5_1deg = xr.open_dataset(obs_fname, engine='cfgrib').sel(latitude=slice(80,20))
...
era5_clim = xr.open_dataset(clim_fname_sf, engine='cfgrib', backend_kwargs={'filter_by_keys': {'shortName': 'msl'}})
...
for f in hcanom.forecastMonth:
    ...
    solver = Eof(cl_array.msl, weights=wgts)
...
for n in hcanom.number:
    ...
    for f in hcanom.forecastMonth:
        ...
        for t in hcanom.start_date:
            ...
            pcs = solver.projectField(hcanom.sel(number=n,forecastMonth=f,start_date=t).msl, neofs=number_of_eofs, eofscaling=1)
...
for n in hcanom_3m.number:
    ...
    for f in hcanom_3m.forecastMonth:
        ...
        for t in hcanom_3m.start_date:
            ...
            pcs = solver.projectField(hcanom_3m.sel(number=n,forecastMonth=f,start_date=t).msl, neofs=number_of_eofs, eofscaling=1)
            list3_hcpcs.append(pcs.assign_coords({'start_date':t}))
        list3 = xr.concat(list3_hcpcs,dim='start_date')                    
        list2_hcpcs.append(list3.assign_coords({'forecastMonth':f}))
    list2 = xr.concat(list2_hcpcs,dim='forecastMonth')                    
...
for t in obanom.valid_time:
    ...
    pcs = solver.projectField(obanom.sel(valid_time=t).msl, neofs=number_of_eofs, eofscaling=1)
...
for t in obanom_3m.valid_time:
    ...
    pcs = solver.projectField(obanom_3m.sel(valid_time=t).msl, neofs=number_of_eofs, eofscaling=1)
    list1_obpcs.append(pcs.assign_coords({'valid_time':t}))
obpcs_3m = xr.concat(list1_obpcs,dim='valid_time')                   

# Saving pcs index to netCDF files
