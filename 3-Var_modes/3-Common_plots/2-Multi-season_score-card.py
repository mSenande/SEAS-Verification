...
# If no variables were introduced, ask for them
    ...
    var = input("Selecciona la variable [ NAO , eof1 , eof2 , eof3 , eof4 ]: ")
...
for label in full_name:
    ...
    for season in var_seasons:
        ...
        for init in initialization:
            ...
            # Read files
            if var=='NAO':
                fname = '{datadir}/1-Box_calc/data/scores/{origin}_s{system}_stmonth{start_month:02d}_hindcast{hcstarty}-{hcendy}_monthly.{aggr}.{score}.nc'.format(datadir=DATADIR,**config)
                scorefile = xr.open_dataset(fname).sel(forecastMonth=fcmonth)
            else:
                fname = '{datadir}/2-EOFs_calc/data/scores/{origin}_s{system}_stmonth{start_month:02d}_hindcast{hcstarty}-{hcendy}_monthly.{aggr}.{score}.nc'.format(datadir=DATADIR,**config)
                scorefile = xr.open_dataset(fname).sel(forecastMonth=fcmonth)
...
figname = f'./Score-card_{score}_{aggr}_{var}.png'
...
csvname = f'./Score-card_{score}_{aggr}_{var}.csv'
df.to_csv(csvname)